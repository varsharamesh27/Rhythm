    ...
    <AppShell>
      <div className="grid gap-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-stone-600">Track routine, recovery, movement, nutrition, and career habits without turning one missed day into a verdict.</p>
        </div>
        <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
          <Card>
            <CardHeader><CardTitle>Today&apos;s habit list</CardTitle></CardHeader>
            <CardContent className="grid gap-3">
              <p className="rounded-md bg-emerald-50 p-3 text-sm font-medium text-emerald-800">Seven-day target progress: {weeklyCompletion}%</p>
              {habits.length === 0 ? <p className="text-sm text-stone-600">Add your first habit to begin tracking.</p> : null}
              {habits.map((habit) => {
