    ...
    <AppShell>
      <div className="grid gap-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-stone-600">Recovery, movement, hydration, nutrition, mood, energy, and optional weight trends. Weight is one signal, not the scoreboard.</p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="overflow-x-auto">
            {checkins.length === 0 ? <p className="text-sm text-stone-600">No health check-ins yet.</p> : (
              <table className="w-full min-w-[680px] text-left text-sm">
                <thead className="text-stone-600"><tr><th className="py-2">Date</th><th>Mood</th><th>Energy</th><th>Water</th><th>Workout</th><th>Walking</th><th>Study</th><th>Weight</th></tr></thead>
                <tbody>{checkins.slice(-10).reverse().map((checkin) => <tr key={checkin.id} className="border-t border-stone-200"><td className="py-3">{checkin.checkin_date}</td><td>{checkin.mood}/5</td><td>{checkin.energy}/5</td><td>{checkin.water_intake}</td><td>{checkin.workout_completed ? "Yes" : "No"}</td><td>{checkin.walking_completed ? "Yes" : "No"}</td><td>{checkin.study_completed ? "Yes" : "No"}</td><td>{checkin.weight ?? "--"}</td></tr>)}</tbody>
              </table>
            )}
          </CardContent>
        </Card>
