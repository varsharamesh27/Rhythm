...
export function CheckinForm({ today, existing }: { today: string; existing: DailyCheckin | null }) {
  ...
  return (
    <Card>
      <CardHeader>
        <p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">Today</p>
        <CardTitle>Daily check-in</CardTitle>
        <p className="text-sm text-stone-600">A calm two-minute pass through routine, recovery, movement, nutrition, and career progress.</p>
      </CardHeader>
      <CardContent>
        <form action={action} className="grid gap-5" data-testid="daily-checkin-form">
          ...
          <fieldset className="grid gap-3 rounded-lg border border-stone-200 p-4">
            <legend className="px-1 text-sm font-semibold text-stone-800">Completed today</legend>
...
function Check({ name, label, checked }: { name: string; label: string; checked: boolean }) {
  return (
    <label className="flex min-h-11 items-center gap-3 rounded-md bg-stone-50 px-3 py-2 text-sm font-medium text-stone-800">
      <input name={name} type="checkbox" defaultChecked={checked} className="size-4 accent-emerald-700" />
      {label}
    </label>
  );
}
